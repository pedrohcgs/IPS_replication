...
source(here("Codes", "quantile_itt.R"))
...
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
# Intention to treat with baseline specification
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
# Propensity scores
#----------------------------------------------------------------------------
...
# Test if the model is correctly specified
#ncores = 4
#spec.test <- pstest::pstest(treat, ps.glm$fitted.values, baselineX, cores = ncores, w = "ind")
#----------------------------------------------------------------------------
...
# IPS with indicator
s.ind <- Sys.time() 
ps.ind <- IPS::IPS_ind(d = treat, x = baselineX, 
                       xbal = xbal,
                       Treated = FALSE)
e.ind <- Sys.time() 
e.ind - s.ind
...
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
# compute Average ITT using different PS estimators
#----------------------------------------------------------------------------
#outcome is total wealth or Net total Financial Assets
tw.aitt <- average_itt(data401k$tw)
write.csv(tw.aitt,
          file =  here("Applications/401k/Results/", 
                       paste0("out_TotalWealth_itt.csv"))
)
#outcome is Net total Financial Assets
net_tfa.aitt <- average_itt(data401k$net_tfa)

write.csv(net_tfa.aitt,
          file =  here("Applications/401k/Results/", 
                       paste0("out_NetTotalFA_itt.csv"))
)

#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
# Estimate Quantile ITT using different PS estimators
#----------------------------------------------------------------------------
# The tau for quantile effects
# CODE BELOW IS FOR these 5 quantile indexes only!!!
tau <- c(0.10, 0.25, 0.5, 0.75, 0.90)
#outcome is total wealth or Net total Financial Assets
tw.qitt <- quantile_itt(data401k$tw, tau)

write.csv(tw.qitt,
          file =  here("Applications/401k/Results/", 
                       paste0("out_TotalWealth_quantile_itt.csv"))
)

#outcome is Net total Financial Assets
net_tfa.qitt <- quantile_itt(data401k$net_tfa, tau)
write.csv(net_tfa.qitt,
          file =  here("Applications/401k/Results/", 
                       paste0("out_NetTotalFA_quantile_itt.csv"))
)
#----------------------------------------------------------------------------
...
#----------------------------------------------------------------------------
# Save Dataset
save.image( here("Applications/401k/Results/401k_itt.RData"))
